...
const KNOWN_NAMES = new Set([
  'jeffrey epstein', 'ghislaine maxwell', 'prince andrew', 'virginia giuffre',
  'virginia roberts', 'alan dershowitz', 'bill clinton', 'donald trump',
  'les wexner', 'jean-luc brunel', 'sarah kellen', 'nadia marcinkova',
  'ehud barak', 'bill richardson', 'george mitchell', 'marvin minsky',
  'glenn dubin', 'eva andersson-dubin', 'leon black', 'mort zuckerman',
  'steve bannon', 'kevin spacey', 'chris tucker', 'naomi campbell',
  'courtney love', 'tony blair', 'larry summers', 'stephen hawking',
  'reid hoffman', 'bill gates', 'woody allen', 'leslie wexner',
  'sarah ransome', 'maria farmer', 'annie farmer', 'johanna sjoberg',
  'maxwell family', 'robert maxwell', 'alexander acosta'
]);
...
async function loadProgress() {
  ...
  return { offset: START_OFFSET, stats: { totalDocs: 0, totalPeople: 0, totalOrgs: 0, totalLocs: 0 } };
}
...
async function main() {
  ...
  while (running) {
    ...
    if (result.processed > 0) {
      ...
      console.log(`[${new Date().toISOString()}] Processed ${result.processed} docs (offset: ${offset}) in ${elapsed}s | ` +
