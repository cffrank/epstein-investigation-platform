	...
	try {
		switch (toolName) {
			...
			case "get_stats": {
				...
				const bySource = await cachedQuery(
					"SELECT source, COUNT(*) as count, COUNT(CASE WHEN embedding_status = 'completed' THEN 1 END) as completed FROM documents GROUP BY source ORDER BY count DESC LIMIT 20",
					[],
				);
