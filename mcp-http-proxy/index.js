...
app.post("/tools/:name", async (c) => {
  ...
  try {
    switch (toolName) {
      ...
      case "list_tables": {
        const result = await pool.query("SELECT table_name, pg_size_pretty(pg_total_relation_size(quote_ident(table_name))) as size, (SELECT reltuples::bigint FROM pg_class WHERE relname = table_name) as row_estimate FROM information_schema.tables WHERE table_schema = 'public' AND table_type = 'BASE TABLE' ORDER BY table_name");
        return c.json({ tables: result.rows });
      }

      case "get_stats": {
        const result = await pool.query("SELECT COUNT(*) as total_documents, COUNT(CASE WHEN search_vector IS NOT NULL THEN 1 END) as indexed, COUNT(CASE WHEN embedding_status = 'completed' THEN 1 END) as with_embeddings, COUNT(CASE WHEN embedding_status = 'pending' THEN 1 END) as pending_embeddings, COUNT(CASE WHEN embedding_status = 'failed' THEN 1 END) as failed_embeddings, COUNT(CASE WHEN ocr_status = 'completed' THEN 1 END) as ocr_completed, COUNT(DISTINCT source) as sources FROM documents");
        const bySource = await pool.query("SELECT source, COUNT(*) as count, COUNT(CASE WHEN embedding_status = 'completed' THEN 1 END) as completed FROM documents GROUP BY source ORDER BY count DESC LIMIT 20");
        return c.json({ overview: result.rows[0], bySource: bySource.rows });
      }
