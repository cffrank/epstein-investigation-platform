...
const config = {
	...
	neo4j: {
		...
		port: Number.parseInt(process.env.NEO4J_BOLT_PORT || "7687"),
		user: process.env.NEO4J_USER || "neo4j",
		password: process.env.NEO4J_PASSWORD || "",
	},
	batchSize: Number.parseInt(process.env.BATCH_SIZE || "50"),
...
// Neo4j driver
const driver = neo4j.driver(neo4jUri, neo4j.auth.basic(config.neo4j.user, config.neo4j.password));
	...
	try {
		for (const doc of documents) {
			...
			try {
				...
				const totalEntities =
					entityCounts.people + entityCounts.organizations + entityCounts.places;
